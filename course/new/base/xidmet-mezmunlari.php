...
<div class="page">
   <div class="page-header">
      <h1 class="page-title">Xidmət Məzmunları</h1>
               ...
               <table class="table table-bordered">
                  ...
                  <tbody>
                     ...
                     <tr align="center">
                        ...
                        <td><?= $xidmetler->Mez_Kat_Getir($m_data[$i]['xid_kat_id']); ?></td>
                        <td><button class="btn btn-outline-info btn-sm" data-toggle="modal" data-target="#mezmun_<?= $m_data[$i]['id'] ?> ">Bax</button></td>
                        <td>
                           <?php if ($m_data[$i]['status']==1): ?>
                           <a href="xidmet-mezmunlari.php?id=<?= $m_data[$i]['id']?>&m_status=deaktiv"><button class="btn btn-danger btn-sm">Deaktiv Et</button></a>
                           <?php endif ?>
                           <?php if ($m_data[$i]['status']==0): ?>
                           <a href="xidmet-mezmunlari.php?id=<?= $m_data[$i]['id']?>&m_status=aktiv"><button class="btn btn-info btn-sm">Aktiv Et</button></a>
                           <?php endif ?>
                        </td>
                        <td>

                            <a href="xidmet-mezmun-redakte.php?data=mredakte&id=<?= $m_data[$i]['id'] ?>"><button class="btn btn-outline-info btn-sm">Redaktə</button></a>
...
<?php for ($i=0; $i < count($m_data) ; $i++) {  ?>
<div class="modal fade" id="mezmun_<?= $m_data[$i]['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
   <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
         <div class="modal-header">
            <h5 class="modal-title" id="exampleModalCenterTitle"><?= $m_data[$i]['Basliq'] ?></h5>
         ...
         <div class="modal-body">
            <div class="row">
               <div class="col-xl-12 form-group">
                  ...
                  <select name="xid_kat" class="form-control">
...
<script>
   function sil(id) {
      ...
      if(x){
         window.location.href = "xidmet-mezmunlari.php?m_status=sil&id="+id;
      }
      else{
         alert("İmtina edildi");
      }
