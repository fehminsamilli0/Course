<?php 
   class Portfolio extends General{
   
   	public function K_Getir()
   	{
   		global $db;
   
   		$getir = $db->prepare("SELECT * from portfolio_kat");
   		$getir->execute();
   	...
   	{
   		global $db;
   		$sil = $db->prepare("DELETE from portfolio_kat
   		 where id=:id");
   	...
   	public function AktivEt($post)
   	{
   		...
   		$deyis = $db->prepare("UPDATE portfolio_kat set status=:s where id=:id");
   	...
   	{
   		global $db;
   		$elave=$db->prepare("INSERT into portfolio_kat set 
   			ad=:ad,
   			sira=:sira,
   			seo=:seo
   	...
   	public function Sira()
   	{
   		...
   		$getir = $db->prepare("SELECT sira from portfolio_kat order by sira ASC");
   		$getir->execute();
   		$massiv = array();
   	...
   	public function M_Getir()
   	{
   		...
   		$getir = $db->prepare("SELECT * from portfolio_mez");
   		$getir->execute();
   	...
   	public function Mez_Kat_Getir($id)
   	{
   		...
   		$getir = $db->prepare("SELECT * from portfolio_kat where id=:id");
   	 ...
   	 public function M_AktivEt($post)
      {
         ...
         $deyis = $db->prepare("UPDATE portfolio_mez set status=:s where id=:id ");
         $x = $deyis->execute(array(
            's'=>1,
            'id'=>$post['id']
         ));
      ...
      {
         global $db;
         $sil = $db->prepare("DELETE from portfolio_mez where id=:id");
         $x = $sil->execute(array('id'=>$post['id']));
         return $x ? 1 : 0;
      }
   	...
   	public function M_ElaveEt($post)
   	{
   		...
   		if($olcu>0){
   			if (in_array($tip,$tipler)) {
   				...
   				$elave=$db->prepare("INSERT into portfolio_mez set 
   					port_kat_id=:kid,
   					Basliq=:bas,
   					sekil=:sekil
   	...
   	public function RMGetir($id)
	{
		...
		$getir = $db->prepare("SELECT * from portfolio_mez where id=:id");
		$getir->execute(array('id'=>$id));
	  ...
	  public function MRedakteEt($post)
   {
      ...
      if($olcu>0){
         if (in_array($tip,$tipler)) {
            ...
            $rnd=rand(1000,9999);
            $sekil_yol=$yol.$rnd."-".$name;
            $redakte=$db->prepare("UPDATE portfolio_mez set 
               port_kat_id=:kid,
               Basliq=:bas,
               sekil=:sekil
               where id=:id
